...
const DEMO_PASSWORD = "HBTS@123";
const DEMO_OTP = "000000";
const DEMO_USERS = {
  "conductor@gmail.com": { role: "conductor", name: "Demo Conductor" },
  "passenger@gmail.com": { role: "passenger", name: "Demo Passenger" },
  "admin@gmail.com": { role: "admin", name: "Demo Admin" },
};

function getDemoUser(identifier) {
  if (!identifier) return null;
  return DEMO_USERS[identifier.trim().toLowerCase()] || null;
}

async function createFixedOtp({ userId, purpose }) {
...
async function ensureDemoUser(identifier, demo) {
  ...
  const roleId = await getRoleId(demo.role);
  ...
  const { rows } = await pool.query(
    ...
    `,
    [demo.name, identifier.trim(), null, passwordHash, roleId]
  );

  return { ...rows[0], role: demo.role, is_verified: true };
}
...
export async function passengerLogin(req, res) {
  try {
    ...
    if (demo?.role === "passenger" && password === DEMO_PASSWORD) {
      const user = await ensureDemoUser(email, demo);
      const otpData = await createFixedOtp({
        userId: user.user_id,
        purpose: "LOGIN_2FA",
      });
...
export async function login(req, res) {
  try {
    ...
    const demo = getDemoUser(identifier);
    if (demo && password === DEMO_PASSWORD) {
      const user = await ensureDemoUser(identifier, demo);
      const otpData = await createFixedOtp({
        userId: user.user_id,
        purpose: "LOGIN_2FA",
      });
...
export const adminLogin = async (req, res) => {
  try {
    ...
    const demo = getDemoUser(email);
    if (demo?.role === "admin" && password === DEMO_PASSWORD) {
      const user = await ensureDemoUser(email, demo);
      const otpData = await createFixedOtp({
        userId: user.user_id,
        purpose: "LOGIN_2FA",
